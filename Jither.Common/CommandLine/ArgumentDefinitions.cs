...
public abstract class ArgumentDefinition
{
    ...
    protected void SetupList()
    {
        if (this.PropertyType.IsGenericType && this.PropertyType.GetInterfaces().Any(i => i == typeof(IEnumerable)))
        {
            ...
            if (genArgs.Length != 1)
            {
                throw new ArgumentException($"{DisplayName}: Generic type {this.PropertyType} isn't supported.");
            }
            var itemType = genArgs[0];
            var listType = typeof(List<>).MakeGenericType(itemType);
            if (!this.PropertyType.IsAssignableFrom(listType))
            {
                throw new ArgumentException($"{DisplayName}: {this.PropertyType} cannot be instantiated.");
            }
            this.IsList = true;
    ...
    public void ApplyList<T>(T obj, IEnumerable<IArgumentValue> options) where T : class, new()
    {
...
public class PositionalDefinition : ArgumentDefinition
{
    ...
    public override string DisplayName => $"{Name} (at position {Position})";
