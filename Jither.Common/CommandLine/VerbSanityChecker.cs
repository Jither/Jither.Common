...
public class VerbSanityChecker
{
    ...
    private void CheckLists(IssueCollection issues, Verb verb)
    {
        ...
        for (int i = 0; i < defs.Positionals.Count; i++)
        {
            var positional = defs.Positionals[i];
            if (positional.IsList && i < defs.Positionals.Count - 1)
            {
                issues.Add(verb, IssueType.PositionalListNotLast, $"{positional.DisplayName} is a list - only the last positional can be a list.");
            }
        }
    }
