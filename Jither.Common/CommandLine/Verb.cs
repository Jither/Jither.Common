...
public class Verb<TOptions> : Verb where TOptions : class, new()
{
    ...
    protected override void ValidateArguments(ArgumentDefinitions definitions, ArgumentValues args)
    {
        ...
        if (values.Count > definitions.Positionals.Count && !definitions.Positionals.Any(p => p.IsList))
        {
    ...
    private void ApplyArguments(TOptions options, ArgumentDefinitions definitions, ArgumentValues values)
    {
        ...
        foreach (var positional in definitions.Positionals)
        {
            if (positional.IsList)
            {
                // Validation already checked that this is, in fact, the last positional, so just
                // take all the remaining arguments
                var vals = values.Positionals.Where(v => v.Position >= positional.Position);
                positional.ApplyList(options, vals);
            }
            else
            {
                var val = values.Positionals.SingleOrDefault(v => v.Position == positional.Position);
                if (val != null)
                {
                    positional.Apply(options, val);
                }
            }
        }
    }
